...
def calc_scale_shift(depth, normal):
    ...
    for scale in range(1, 50, 2):
        for shift in range(11, 19, 2):
            depth_affine = depth * scale / 10. + shift / 10.
            depth2normal = surface_normal_from_depth(torch.from_numpy(depth_affine[None, None]).to(torch.float32).cuda(), torch.Tensor(focal_length).cuda())
            depth2normal = depth2normal.cpu().numpy()[0].transpose(1, 2, 0)

            normal_error = torch.cosine_similarity(torch.from_numpy(depth2normal), torch.from_numpy(normal), dim=-1)
            normal_error = torch.clamp(normal_error, min=-1.0, max=1.0)
            angle_error = torch.acos(normal_error) * 180.0 / torch.pi
            err = angle_error.mean().detach().cpu().item()
    ...
    return scale_best/10. , shift_best/10.
